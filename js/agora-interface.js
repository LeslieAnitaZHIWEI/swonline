...
function initClientAndJoinChannel(agoraAppId, channelName) {
  ...
  // client= AgoraRTC.createClient({mode: option.mode, codec: option.codec});
  // init Agora SDK
...
function joinChannel(channelName) {
  ...
  var userID = JSON.parse(getCookie(getQueryVariable('id'))).userId
...
function createCameraStream(uid) {
  var option = serializeformData(); // Data is feteched and serilized from the form element.
...
function joinChannelAsScreenShare() {
  ...
  screenClient.on('stream-published', function (evt) {
    ...
    // localStreams.play('full-screen-video'); // play the screen share as full-screen-video (vortext effect?)
...
function leave (rtc) {
  ...
  client.leave(function () {
    ...
    for (let i = 0; i < remoteStreams.length; i++) {
      var stream = remoteStreams.shift()
      var id = stream.getId()
      if(stream.isPlaying()) {
        stream.stop()
      }
...
function generateToken() {
  return JSON.parse(getCookie(getQueryVariable('id'))).token
}